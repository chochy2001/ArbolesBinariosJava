...
public class BTree {//Clase que nos sirve para hacer la implementacion de los arboles B
    ...
    private BNode leafNode( BNode nodo, int n ){//Metodo para con base en un valor, agregarlo a la hoja
        if( nodo.leaf ){//Si el nodo tiene nojas se regresa el nodo
            ...
            for(; i < nodo.key.size() ; i++ ){//Se recorre con base al tamaño de llaves
                if( n < nodo.getKey(i) ){//Si el valor ingresado es menor al valor de la llave se aumenta su valor
                    // (Ya que estan ordenados de menos a mayor)
    ...
    private void insert( BNode nodo, int n ){//Método insertar, se manda en valor del valor y
        // el nodo en donde se quiere insertar el valor
        int i = 0;
    ...
    private void divisionCelular( BNode nodo, int n ){//Recibimos el nodo y el valor que queremos agregar
        ...
        int h = (m-1)/2;   //Se hace la resta de 1 y se divide entre 2 para poder determinar el valor donde
        // m son los valores del nodo
        if( m % 2 != 0){ //Si m modulo 2 es diferente de cero
            insert(nodo,n);//Se inserta el nodo con el numero pasado al metodo
        }
        int medio = nodo.key.get(h);//Se determina el valor medio del arbol, con ayuda de h que es lo que
        // esta en medio y se asigna a medio
            
        ArrayList<Integer> key1 = new ArrayList( nodo.key.subList( 0, h ) );
        //Se crea un nuevo arraylist de enteros llamado key 1 donde se inicializara con el valor de la
        // sublista desde el principio del arbol hasta la mitad.
        ArrayList<Integer> key2 = new ArrayList( nodo.key.subList( h + 1 , 2*h + 1) );
        //Se crea un nuevo arraylista de enteros llamado key 2 donde se inicializara con el valor de la
        // sublista desde el valor intermedio mas 1 hasta el doble de h más 1
        ...
        if( nodo == root ){//Si el nodo es igual a la raiz
            ...
            nuevoNodo1.leaf = nuevoNodo2.leaf = nodo.leaf;//Duplicamos el valor del la hoja del nodo
            // hacia los nuevos nodos
            ...
            if( !nodo.leaf ){//Si las hojas es diferente al nodo
                if( m % 2 != 0 ){//Si el modulo 2 es diferente de cero
                    child1 = new ArrayList( nodo.child.subList( 0, h+1 ) );//creamos un nuevo arraylist
                    // para guardar la primera mitad del nodo
                    child2 = new ArrayList( nodo.child.subList( h+1, m+1 ) );//creamos un nuevo arraylist
                    // para guardar la segunda mitad del nodo
                }
                else{//Si el modulo da como resultado 0
                    if( n < medio ){//Si el valor ingresado es menor al medio
                        child1 = new ArrayList( nodo.child.subList( 0, h+2 ) );//creamos un nuevo arraylist
                        // para guardar la primera mitad del nodo
                        child2 = new ArrayList( nodo.child.subList( h+2, m+1 ) );//creamos un nuevo arraylist
                        // para guardar la segunda mitad del nodo
                    }
                ...
                for( BNode i : nuevoNodo1.child )
                    i.parent = nuevoNodo1;//Recorremos todo el nodo con su hijo para pasarle los valores
                // al parent del nuevoNodo2
                for( BNode i : nuevoNodo2.child )
                    i.parent = nuevoNodo2;//Recorremos todo el nodo con su hijo para pasarle los valores
                // al parent del nuevoNodo2
            ...
            nuevoNodo1.parent = nuevoNodo2.parent = root;//Se hace la asignacion de la raiz hacia los nodos
            // hijos nuevos
            ...
            int childIndex = nodo.getChildIndex();//Creamos una variable para poder acceder a la cantidad de
            // indices que tiene el nodo.
    ...
    public void mostrarArbol(){
        if(root.child.isEmpty() && root.key.isEmpty()){//Si la raiz del arbol y las llaves estan vacias
            // significa que esta vacio el arbol
        ...
        while( !nodos.isEmpty() ){//Mientras que no este vacio la cola
            ...
            BNode v = nodos.poll();//Vamos a ir sacando los elementos de la misma y almacenandolos en la variable
            // v que es un nodoB
    ...
    public boolean find(int value){//Método que nos sirve para encontrar un determinado valor en el arbol
        if(root.child.isEmpty() && root.key.isEmpty()) return false;//Se hizo una simplificacion del
        // codigo por redundancias y obviedades
    ...
    private boolean find(int v,BNode n){//Metodo que nos sirve para buscar un valor en un nodo en concreto
        if(n==null) return false;//Si el nodo esta vacio retornamos false, ya que no puede haber ningun
        // valor en un arbol vacio.
        
        int i;
        
        if( n.getKey(0) > v )//Si el valor obtenido en el indice 0 es mayor al valor ingresado
            return find(v,n.getChild(0));//Retornamos el valor de la busqueda de nuevo con el valor
        // a buscar y el hijo en el indice 0
        
        for(i=0;i<n.key.size()-1;i++){// Se hace un ciclo desde cero hasta el tamaño de las llaves menos
            // 1 para saber donde colocarlo
            
            if(n.getKey(i)==v){//Si el valor se encuentra en el arbol regresas true
                return true;
            }
            
            if(n.getKey(i)<v && n.getKey(i+1)>v){ //Si la clave obtenida es menor al valor y la clave
                // y mayor a la anterior
